...
class MDictBase {
    ...
    readDict() {
        ...
        this._readKeyInfos();
        // STEP4: read key block (Lazy loaded on demand, do not read all blocks)
        // this._readKeyBlocks();
        // STEP5: read record header
        this._readRecordHeader();
        // STEP6: read record block info
        this._readRecordInfos();
        // Initialize keywordList as empty
        this.keywordList = [];
    }
    /**
    ...
    splitKeyBlock(keyBlock, keyBlockIdx) {
        ...
        if (keyList.length > 0) {
            ...
            if (keyBlockIdx + 1 < this.keyInfoList.length) {
                const nextIdx = keyBlockIdx + 1;
                const packSize = this.keyInfoList[nextIdx].keyBlockPackSize;
                const unpackSize = this.keyInfoList[nextIdx].keyBlockUnpackSize;
    ...
    _readHeader() {
        ...
        // if (!(header_b_cksum)) throw new Error("Assertion failed: header_b_cksum"), "header_bytes checksum failed");
    ...
    unpackKeyBlock(kbPackedBuff, unpackSize) {
        ...
        if (compType === 0) {
            ...
            // if (!(adler32 == zlib.adler32(key_block)) throw new Error("Assertion failed: adler32 == zlib.adler32(key_block") & 0xffffffff)
        }
        else {
...
export class Mdict extends MDictBase {
    ...
    lookupKeyBlockByWord(word, isAssociate = false) {
        ...
        const list = this.lookupPartialKeyBlockListByKeyInfoId(keyBlockInfoId);
        // binary search
        let left = 0;
    ...
    lookupRecordByKeyBlock(item) {
        ...
        if (!this._recordBlockCache) {
            this._recordBlockCache = new Map();
        }
        let unpackRecordBlockBuff = this._recordBlockCache.get(recordBlockIndex);
        if (!unpackRecordBlockBuff) {
            ...
            if (this._recordBlockCache.size > 20) {
                ...
                if (firstKey !== undefined) {
                    this._recordBlockCache.delete(firstKey);
                }
            }
        }
        const recordBlockInfo = this.recordInfoList[recordBlockIndex];
    ...
    decompressBuff(recordBuffer, unpackSize) {
        // decompress
        // 4 bytes: compression type
        const rbCompType = recordBuffer[0];
        // record_block stores the final record data
...
export class MDX extends Mdict {
    ...
    searchPrefix(prefix) {
        ...
        for (let idx = 0; idx < this.keyInfoList.length; idx++) {
            ...
            if (cleanLast >= cleanedPrefix && cleanFirst <= cleanedPrefix + '\uffff') {
                const partialList = this.lookupPartialKeyBlockListByKeyInfoId(idx);
                for (const item of partialList) {
                    const cleanKey = item.keyText.toLowerCase().trim();
                    if (cleanKey.startsWith(cleanedPrefix)) {
