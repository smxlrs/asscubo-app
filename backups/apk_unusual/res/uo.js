...
function _getFareName() {
    var e = $("#selectedOfferLabel00Text");
    return e ? e.attr("title") : "";
}
...
function addPurchaseHook() {
    ...
    if (e) {
        ...
        e.attr("onclick", "orariotreni.setPayMethod(_getPayMethod()); orariotreni.setEmail(_getEmail());"+onclick);
    }
}
...
function ot_fixInputs() {
    $(function() {
        ...
        $('input[id$="travellerParameterVal2"]').removeAttr('onkeypress').removeAttr('onkeyup').removeAttr('onmousedown').removeAttr('onfocus');
     });
}

function ot_setUserInfoSubscription(name, surname, subscription) {
    $(function() {
         $('#nameSubscriptionId').val(name);
         $('#surnameSubscriptionId').val(surname);
         $('#numSubscriptionId').val(subscription);
         if (name.length > 0 && surname.length > 0 && subscription.length > 0) {
            $('#prenotaSubscriptionButtonId').click();
         }
    });
}

function ot_setUserInfoCarnet(name, surname, carnet) {
    $(function() {
         $('#nameCarnetId').val(name);
         $('#surnameCarnetId').val(surname);
         $('#numCarnetId').val(carnet);
    });
}

function ot_setUserInfoRegionalCarnet(name, surname, carnet) {
    $(function() {
         $('#idNameCarnet').val(name);
         $('#idSurnameCarnet').val(surname);
         $('#idNumCarnet').val(carnet);
    });
}
...
function ot_addSearchHookSubscriptions() {
    $(function() {
        var e = $('#cercaSubscription');
        if (e) {
            var old = e.attr('onclick');
            e.attr('onclick', "orariotreni.setOriginDestination($('#origin').val(), $('#destination').val());"+old);
        }
    });
}

function ot_addSearchHookCarnet() {
    $(function() {
        var e = $('#acquistaCarnetButtonId');
        if (e) {
            var old = e.attr('onclick');
            e.attr('onclick', "orariotreni.setOriginDestination($('#originCarnetId').val(), $('#destinationCarnetId').val());"+old);
        }
    });
}

function ot_setStations(origin, destination) {
    $(function() {
        $('#origin').val(origin);
...
function ot_setCarnetStations(origin, destination) {
    $(function() {
        $('#originCarnetId').val(origin);
        $('#destinationCarnetId').val(destination);
    });
}